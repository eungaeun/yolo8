...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, GGIoU=False, alpha=0.3, beta=1/7, eps=1e-7):
    """
    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                return iou - (rho2 / c2 + v * ciou_alpha)  # CIoU
            return iou - rho2 / c2  # DIoUdd
        c_area = cw * ch + eps  # convex area
        return iou - (c_area - union) / c_area  # GIoU https://arxiv.org/pdf/1902.09630.pdf
    
    if GGIoU:
            sigma1 = beta * (b1_x2.minimum(b2_x2) - b1_x1.maximum(b2_x1)).clamp_(0) + eps
...
class DetMetrics(SimpleClass):
    ...
    def maps(self):
        ...
        with open(os.path.join(self.save_dir / "mAP.txt"), "w") as f:
            f.write(f"mAP: {self.box.maps.mean():.4f}\n")  # 전체 mAP 저장
            for i, map_value in enumerate(self.box.maps):
                f.write(f"Class {i}: {map_value:.4f}\n")  # 클래스별 mAP 저장
